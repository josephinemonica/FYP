...
def IK(x_d,z_d,q_1,Time):
    ...
    for k in range(1,N+1):  #k=1,2,3, ... ,N
        ...
        R_sys=0.3   #Radius of sphere enclosing the system (UAV+arm)
        W_oa=0      #Weight of the Obstacle avoidance task
        J_oa=-Matrix([[q_k[0]-x_obs,q_k[1]-y_obs,q_k[2]-z_obs,0,0,0,0,0]]) #Jacobian of the Obstacle avoidance task
        
        d=sqrt( (q_k[0]-x_obs)**2+ (q_k[1]-y_obs)**2 +(q_k[2]-z_obs)**2) #Distace between the 2 sphere (obstacle & sys) centers
